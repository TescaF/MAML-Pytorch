...
def main():
    ...
    for epoch in range(args.epoch):
        pos_keys,neg_keys,batch_y = db_train.next()
        spt_keys, qry_keys = [], []
        for k in pos_keys:
        ...
        test_loss, loss_report = maml.forward(neg_keys, spt_keys, y_spt, qry_keys, y_qry)
        ...
        if epoch % 500 == 0:  # evaluation
            ...
            test_losses,ft_training,pt_training = [],[],[]
            pos_keys,neg_keys,batch_y = db_test.next()
            y_spt = batch_y[:,:k_spt,:]
            y_qry = batch_y[:,k_spt:,:]
            y_spt, y_qry = torch.from_numpy(y_spt).float().to(device), torch.from_numpy(y_qry).float().to(device)

            t = 0
            for y_spt_one, y_qry_one, pos_one, neg_one in zip(y_spt,y_qry,pos_keys,neg_keys):
                spt_keys = pos_one[:k_spt]
                qry_keys = pos_one[k_spt:]
                loss_report, pred = maml.tune(neg_one, spt_keys,y_spt_one,qry_keys,y_qry_one)
