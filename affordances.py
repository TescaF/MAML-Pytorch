...
class Affordances:
    def __init__(self, batchsz, k_shot, k_qry, dim_out):
        ...
        self.scale = np.array([640,480,math.pi])
        self.affs = []
        data_count = 0
        fts_loc = "/home/tesca/data/part-affordance-dataset/features/reduced_fts_0.95.pkl"
        for aff in range(2,7):
            aff_loc = "/home/tesca/data/part-affordance-dataset/features/all_small_aff/aff_" + str(aff) + "_positions.pkl"
    ...
    def next(self):
        ...
        for t in range(self.batch_size):
            ...
            s = self.rand.uniform(0.0, 1.0, [self.dim_latent, self.dim_output])
            i = self.rand.uniform(0.0, 1.0, self.dim_output)
            ...
            for j in range(self.num_samples_per_class):
                ...
                data = np.array([aff_data[key][-1]/self.scale])
