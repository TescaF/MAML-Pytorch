...
class Affordances:
    def __init__(self, train, batchsz, k_shot, k_qry, dim_out):
        ...
        data_count = 0
        fts_loc = "/home/tesca/data/part-affordance-dataset/features/polar/polar_fts.pkl"
        #fts_loc = "/home/tesca/data/part-affordance-dataset/features/reduced_fts_0.95.pkl"
        training_keys = []
        all_vals = []
        for aff in range(2,7):
            aff_loc = "/home/tesca/data/part-affordance-dataset/features/polar/polar_aff_" + str(aff) + "_positions.pkl"
            with open(aff_loc, 'rb') as handle:
                aff_data = pickle.load(handle)      #dict(category) = [img1, img2, ...]
            keys = list(sorted(aff_data.keys()))
            train_valid_keys = [k for k in keys if (aff_data[k][-1] is not None) and (not k.split("_00")[0].endswith("01"))]
    ...
    def next(self):
        ...
        for t in range(self.batch_size):
            ...
            for j in range(self.num_samples_per_class):
                ...
                vals.append(tf_data)
                outputs[t,j] = self.sc1.transform(tf_data)
