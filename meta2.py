...
class Meta(nn.Module):
    ...
    Meta Learner
    """
    def __init__(self, args, config, loss_fn=F.cross_entropy, accs_fn=torch.eq, al=True):
        """
...
#        self.w = SummaryWriter()
    ...
    def forward(self, x_spt, y_spt, x_qry, y_qry):
        ...
        for i in range(task_num):
            ...
            for k in range(self.update_step):
                ...
                if self.pair:
                    ...
                    for j in range(al_inputs.shape[0]):
                        logits_data = self.al_net(al_inputs[j], vars=al_weights, bn_training=True)
            ...
            with torch.no_grad():
                m_weights[-2] = torch.nn.Parameter(torch.cat([m_weights[-2][:-1],al_weights[0]]))
                m_weights[-1] = torch.nn.Parameter(torch.cat([m_weights[-1][:-1],al_weights[1]]))
        ...
        loss_al = 3.0 * losses_al / task_num
        ...
        return accs, al_accs, loss_q.item(), loss_al.item(), total_loss.item()
    ...
    def al_test(self, x, y, w=None):
        ...
        if self.pair:
            logits = []
            for j in range(al_inputs.shape[0]):
                logits_data = self.net(al_inputs[j], vars=w, bn_training=True, start_idx=16, start_bn=8)
                logits.append(logits_data[0] - logits_data[1])
            logits = torch.stack(logits)
        else:
            logits = self.net(al_inputs, vars=w, bn_training=True, start_idx=16, start_bn=8)
        if self.an > 0:
